...
import { ChevronRight, Calendar, Users, MapPin, Clock, X, Sparkles, ChevronDown, ChevronUp } from 'lucide-react';
import eventsData from "@/data/events";

// Get only flagship events
...
export default function FlagshipEventsSection() {
  ...
  const handleEventSelect = (event: typeof eventsData[0]) => {
    ...
    setShowAllImages(false); // Reset show all images when switching events
                  ...
                  {activeEvent.pastImages.length > 0 && (
                    <div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                        {(showAllImages ? activeEvent.pastImages : activeEvent.pastImages.slice(0, 6)).map((image, index) => (
                          <motion.div
                            key={index}
                      ...
                      {activeEvent.pastImages.length > 6 && (
                        ...
                        >
                          {showAllImages ? (
                            ...
                            <>
                              Show {activeEvent.pastImages.length - 6} More Photos <ChevronDown className="w-4 h-4" />
                            </>
                          )}
                        </button>
                      )}
